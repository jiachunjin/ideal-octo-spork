...
def main():
    ...
    exp_dir = "/data/phd/jinjiachun/experiment/query_dit/0731_hat_training"
    ...
    diffhead_ckpt = torch.load(os.path.join(exp_dir, "diff_head-query_dit-8000"), map_location="cpu", weights_only=True)
    janus.diff_head.load_state_dict(diffhead_ckpt, strict=True)

    siglip16_aligner_ckpt = torch.load(os.path.join(exp_dir, "siglip16_aligner-query_dit-8000"), map_location="cpu", weights_only=True)
    janus.siglip16_aligner.load_state_dict(siglip16_aligner_ckpt, strict=True)
    
    llm_ckpt = torch.load(os.path.join(exp_dir, "janus-backbone-query_dit-8000"), map_location="cpu", weights_only=True)
    ...
    for img_idx, prompt in enumerate(prompts):
        ...
        os.makedirs("asset/hat_training", exist_ok=True)
        torchvision.utils.save_image(grid, f"asset/hat_training/coarse_{img_idx:02d}.png")

        import torchvision.utils as vutils
        sample_path = f"asset/hat_training/fine_{img_idx:02d}.png"
