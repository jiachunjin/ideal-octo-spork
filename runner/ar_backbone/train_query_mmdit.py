...
def main(args):
    ...
    janus, train_scheduler = equip_mmdit_query_with_janus(janus, config)

    if config.train.dit_resume_path is not None:
        dit_ckpt = torch.load(config.train.dit_resume_path, map_location="cpu", weights_only=True)
            ...
            with accelerator.accumulate([janus]):
                ...
                with torch.no_grad():
                    if config.train.gen_feature == "siglip16":
                        ...
                        x_0 = rearrange(x_siglip_dimdown, "b (h w) c -> b c h w", h=24, w=24)
                    elif config.train.gen_feature == "vae":
