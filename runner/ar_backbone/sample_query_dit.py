...
@torch.no_grad()
def main():
    exp_dir = "/data/phd/jinjiachun/experiment/query_dit/0716_query_dit"
    ...
    query_dit_ckpt = torch.load(os.path.join(exp_dir, "query_dit-query_dit-78000"), map_location="cpu", weights_only=True)
    janus.query_dit.load_state_dict(query_dit_ckpt, strict=True)

    query_ckpt = torch.load(os.path.join(exp_dir, "query-query_dit-78000"), map_location="cpu", weights_only=True)
