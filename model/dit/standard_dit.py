...
class LabelEmbedder(nn.Module):
    """
    The dropout is only down by the root transformer
...
class FinalLayer(nn.Module):
    ...
    def __init__(self, hidden_size, out_channels):
        '''
        produce_condition: whether the current transformer should produce output with the same shape as the input or produce a condition for the next transformer
        '''
        super().__init__()
...
if __name__ == "__main__":
    ...
    config = OmegaConf.load("config/clip_dit/clip_dit.yaml")
    model = DiT(config.dit)
