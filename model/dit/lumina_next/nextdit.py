...
from model.dit.lumina_next.lunima_nextdit2d import LuminaNextDiT2DModel
...
if __name__ == "__main__":
    ...
    print(model.model.patch_embedder)
    model.to("cuda")
    model.eval()

    x = torch.randn(1, 1024, 32, 32).to("cuda")
    timestep = torch.randint(0, 1000, (1,)).to("cuda")
    z_latents = torch.randn(1, 64, 3584).to("cuda")
    model_pred = model(x, timestep, z_latents)
